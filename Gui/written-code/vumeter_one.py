...
class VisualizerVerti(object):
    ...
    def generate(self):
        ...
        if self.stream.get_read_available():
            self.data = self.stream.read(self.chunk)
        levels = self.calculate_levels(self.data, self.chunk, self.samplerate, matrix_width)
        for i, level in enumerate(levels):
            self.color = color_convert(interp_color(level/max(levels)))
            level = max(min(level/self.scale, 1.0), 0.0)
            level = level**self.exponent
            level = int(level*0xff)
            self.graphics.drawLine(matrix_width-i-1, 0, matrix_width-i-1, level, self.color)
        return self.graphics.getSurface()
